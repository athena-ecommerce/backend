...
from SCHEMAS.address_schema import EnderecoCompleto, EnderecoResponse

user_router = APIRouter(prefix="/user", tags=["Usuarios"])


@user_router.get("/profile")
async def buscar_informacoes_usuario(db: Session = Depends(pegar_sessao), usuario: Usuarios = Depends(verificar_token), response_model=UserResponse):
    info_usuario = (
        db.query(Usuarios)
